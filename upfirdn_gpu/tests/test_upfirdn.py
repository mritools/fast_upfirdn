...
@pytest.mark.parametrize(
    ...
    product(
        [np.float32, np.float64, np.complex64, np.complex128, np.float16,
         np.uint8, np.int16, np.int32, int],
...
def test_dtype_combos(dtype_data, dtype_filter):
    ...
    # up=1 kernel case
    testing.assert_allclose(upfirdn_scipy(h_cpu, x_cpu, up=1, down=2),
                            upfirdn(h, x, up=1, down=2))

    # down=1 kernel case
    testing.assert_allclose(upfirdn_scipy(h_cpu, x_cpu, up=2, down=1),
...
def test_down(down):
    ...
    # up=1 kernel case
    testing.assert_allclose(upfirdn_scipy(h_cpu, x_cpu, up=1, down=down),
...
def test_up(up):
    ...
    # up=1 kernel case
    testing.assert_allclose(upfirdn_scipy(h_cpu, x_cpu, up=up, down=1),
...
#     %timeit t_cpu = upfirdn_scipy(h_cpu, x_cpu, up=up, down=down, axis=-1)
#     %timeit t0_cpu = upfirdn_scipy(h_cpu, x_cpu, up=up, down=down, axis=0)

#     %timeit t = upfirdn(h, x, up=up, down=down, axis=-1)
#     %timeit t0 = upfirdn(h, x, up=up, down=down, axis=0)
