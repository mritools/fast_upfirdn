...
def test_convolve1d(dtype_x, dtype_h, len_x, mode, xp):
    ...
    for len_h in range(1, len_x):
        ...
        for origin in range(min_origin, max_origin + 1):
            y = ndi.convolve1d(x_cpu, h_cpu, mode=mode, cval=0,
                               origin=origin)

            # test via convolve1d
            y3 = convolve1d(xp.asarray(x_cpu), xp.asarray(h_cpu),
                            mode=mode, cval=0, origin=origin)
        ...
        for origin in [min_origin - 1, max_origin + 1]:
            with pytest.raises(ValueError):
                convolve1d(xp.asarray(x_cpu), xp.asarray(h_cpu), mode=mode,
                           cval=0, origin=origin)
...
def test_correlate1d(dtype_x, dtype_h, len_x, mode, xp):
    ...
    for len_h in range(1, 2*len_x + 2):  # include cases for len_h > len_x
        ...
        for origin in range(min_origin, max_origin + 1):
            y = ndi.correlate1d(x_cpu, h_cpu, mode=mode, cval=0,
                                origin=origin)

            # test via convolve1d
            y3 = correlate1d(xp.asarray(x_cpu), xp.asarray(h_cpu),
                             mode=mode, cval=0, origin=origin)
            xp.testing.assert_allclose(y, y3)

        for origin in [min_origin - 1, max_origin + 1]:
            with pytest.raises(ValueError):
                correlate1d(xp.asarray(x_cpu), xp.asarray(h_cpu), mode=mode,
                            cval=0, origin=origin)
